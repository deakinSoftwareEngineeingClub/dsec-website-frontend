...
export const useViewTransition = () => {
	...
	function createSVGOverlay(): HTMLDivElement {
		let overlay = document.querySelector<HTMLDivElement>(".page-transition-overlay");
		if (!overlay) {
	...
	function slideInOut(href: string, onRouteChange?: () => void): void {
		const overlay = createSVGOverlay();
		const overlayPath = overlay.querySelector<SVGPathElement>(".overlay__path");
